...
build.lcdb.RegTables <- function(){
  ...
  for(i in 1:(length(dates)-1)){
    ...
    TSFR <- getTSR(TSF,dure = months(1))
    re <- reg.TSFR(TSFR,regType='glm')

    if(i==1){
      dbWriteTable(con,'Reg_FactorRtn',transform(re$frtn,date=rdate2int(date),date_end=rdate2int(date_end)),overwrite=T,append=F,row.names=F)
      dbWriteTable(con,'Reg_Residual',transform(re$residual,date=rdate2int(date),date_end=rdate2int(date_end)),overwrite=T,append=F,row.names=F)
      dbWriteTable(con,'Reg_Rsquare',transform(re$Rsquare,date=rdate2int(date),date_end=rdate2int(date_end)),overwrite=T,append=F,row.names=F)
      dbWriteTable(con,'Reg_Pfpwgt',transform(re$pfpwgt,date=rdate2int(date)),overwrite=T,append=F,row.names=F)
    }else{
      dbWriteTable(con,'Reg_FactorRtn',transform(re$frtn,date=rdate2int(date),date_end=rdate2int(date_end)),overwrite=F,append=T,row.names=F)
      dbWriteTable(con,'Reg_Residual',transform(re$residual,date=rdate2int(date),date_end=rdate2int(date_end)),overwrite=F,append=T,row.names=F)
      dbWriteTable(con,'Reg_Rsquare',transform(re$Rsquare,date=rdate2int(date),date_end=rdate2int(date_end)),overwrite=F,append=T,row.names=F)
      dbWriteTable(con,'Reg_Pfpwgt',transform(re$pfpwgt,date=rdate2int(date)),overwrite=F,append=T,row.names=F)
    }
...
#'
#' @export
update.lcdb.RegTables <- function(){
  con <- db.local()
